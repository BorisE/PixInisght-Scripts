/*
   ...
   Improved by Boris Emchenko 2018
...
#feature-info  An automated channel extraction utility.<br/>\
   <br/>\
   A script designed to extract the color channels from RGB images and save them to selected directories.<br/>\
   <br/>\
   Copyright &copy; 2010-2017 S J Brown<br/>\
   Contributions: Copyright &copy; 2017 Michael Covington, 2018 Boris Emchenko
...
this.exportParameters = function() 
{
   ...
   Parameters.set("extractRed", data.extractRed);
   Parameters.set("extractGrn", data.extractGrn);
   Parameters.set("extractBlu", data.extractBlu);
   Parameters.set("redOPD", data.redOPD);
   Parameters.set("grnOPD", data.grnOPD);
   Parameters.set("bluOPD", data.bluOPD);
   Parameters.set("opfExt", data.opfExt);
   for ( var i = 0; i < data.inputFiles.length; ++i ) Parameters.set( "File_"+i, data.inputFiles[i] );


	if( DEBUG ) {
		console.writeln( "<b>Parameters to save:</b>" );
		console.writeln( "R: " + data.extractRed );
		console.writeln( "G: " + data.extractGrn );
		console.writeln( "B: " + data.extractBlu );
		console.writeln( "R: " + data.redOPD );
		console.writeln( "G: " + data.grnOPD );
		console.writeln( "B: " + data.bluOPD );
		console.writeln( "Ext: " + data.opfExt );
	
	    for ( var i = 0; i < data.inputFiles.length; ++i ) console.writeln( "File_"+i + ": " + data.inputFiles[i] ); 
...
this.importParameters = function() 
{
   ...
   if(Parameters.has("extractRed"))
      data.extractRed = Parameters.getBoolean("extractRed");
   if(Parameters.has("extractGrn"))
      data.extractGrn = Parameters.getBoolean("extractGrn");
   if(Parameters.has("extractBlu"))
      data.extractBlu = Parameters.getBoolean("extractBlu");
   
   if(Parameters.has("redOPD"))
      data.redOPD = Parameters.getString("redOPD");
   if(Parameters.has("grnOPD"))
      data.grnOPD = Parameters.getString("grnOPD");
   if(Parameters.has("bluOPD"))
      data.bluOPD = Parameters.getString("bluOPD");

   if(Parameters.has("opfExt"))
      data.opfExt = Parameters.getString("opfExt");

   for ( var i = 0; Parameters.has( "File_"+i) ; ++i )
   {
      data.inputFiles[i] = Parameters.getString( "File_"+i);
...
function mainDialog()
{
   ...
   with(this.inputFiles_TreeBox)
   {
	  ...
	  console.writeln("<br>Fill in TreeBox. Count ",  data.inputFiles.length);
      for ( var i = 0; i < data.inputFiles.length; ++i )
      {
         ...
         node.setText(1, data.inputFiles[i]);
   ...
   this.dialog.redOPD_Edit.text = data.redOPD;
   ...
   this.dialog.grnOPD_Edit.text = data.grnOPD;
   ...
   this.dialog.bluOPD_Edit.text = data.bluOPD;
   ...
   ttStr = "test";
   this.test_Button = new pushButton(this, "Test", "", ttStr);
   this.test_Button.onClick = function()
   {
      runSync("C:/Program Files/PixInsight/src/scripts/BatchFormatConversion\BatchFormatConversion.js");
   }
...
{
	 console.writeln("Running: " + program);
	...
	if (p.exitCode == 0)
	{
		 console.writeln("Finished succesfully");
	}
	else
	{
		 console.writeln("Finished with error: "+ p.ExitCode);
	}

	 console.writeln(p.stdout);
}
